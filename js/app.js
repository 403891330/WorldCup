App = {
  ...
  //分享奖金(需要更改)
  // $(document).on('click', '#sharepool', App.shareAmount);
  ...
  //批量分享奖金
  // $(document).on('click', '#sharesprice', App.SharesAmount);
},
...
//-----------创建比赛--------------
  handleTransfer: function(event) {
    ...
    for(var i = 0;i<price.length;i++){
       // var end_price =  price[i] / 1000000000000000000 ;
      tPrice.push(web3.toWei(price[i]));//web3.toWei(price[i])
    }
    //传入球队id
...
//---------------------------获取总奖金池(根据场次来的)-------------------
    ...
    //传入game id数组获得总价
    getPoolTotal:function(gameids)
    {
       var totalETH =0;
      for(var i = 0;i<gameids.length;i++)
      {
       var totalPrice = App.getPoolFun(gameids[i]);
        totalETH +=totalPrice;
      }
    ...
    getPoolFun:function(gameId)
    {
      var tutorialTokenInstance;
      App.contracts.TutorialToken.deployed().then(function(instance) {
        tutorialTokenInstance = instance;
        // 传入对应的场次ID
        return tutorialTokenInstance.getPool(gameId);
...
//---------------------------设置交易手续费-------------------
  handelSeviceCharge:function(event){
    event.preventDefault();
    var proportion = document.getElementById('charge_price').value;
    //设置手续费百分比比例
    App.handelSeviceChargeFun(proportion);
  },

  handelSeviceChargeFun:function(proportion)
  {
    var tutorialTokenInstance;
    web3.eth.getAccounts(function(error, accounts) {
      if (error) {
      ...
      App.contracts.TutorialToken.deployed().then(function(instance) {
        ...
        return tutorialTokenInstance.setSeviceCharge(proportion, {from: account, gas:700000});
      }).then(function(result) {
        alert('设置手续费成功!')
      }).catch(function(err) {
        console.log(err.message);
  ...
  handelTeamPriceFun:function(teams,tPrice,gameid)
  {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.TutorialToken.deployed().then(function(instance) {
        tutorialTokenInstance = instance;
        // 3 对应的场次id 本地生成累加
        return tutorialTokenInstance.setTeamPrice(teams,tPrice,gameid, {from: account, gas:700000});
      }).then(function(result) {
        alert('设置批量球队价格成功!');
      }).catch(function(err) {
        console.log(err.message);
...
///-------------传入玩法参数返回对应的场次ID----------
  ...
  var type =  document.getElementById("getteam_txt").value;
   App.getGameListFun(type).then(function(result)
  {
    console.log(result);
  });
  },
...
///------------------根据场次ID，获取该场比赛的信息---------
  getListTeamById:function(event){
     ...
     var gameinfo =  App.getListTeamByIdFun(gameid).then(function(result){
       console.log(result);
     }).catch(function(err){
       console.log(err.message);
     });
  ...
  getListTeamByIdFun:function(gameid)
  {
    return new Promise((resolve,reject)=>{
      ...
      App.contracts.TutorialToken.deployed().then(function(instance) {
        ...
        return tutorialTokenInstance.getListTeam(gameid);
      }).then(function(result) {
        resolve(result);
      }).catch(function(err) {
        reject(err);
...
//---------------------------玩法一 购买球队-------------------
  ...
  var total =  document.getElementById("buyteam_total").value;
   ...
   var price =  parseFloat(total) * parseInt(zhushu);
    ...
    App.buyTeamOneFun(gameid,teamsid[0],teamsid[1],teamsid[2],zhushu,price).then(function(result){
      console.log(result);
    }).catch(function(err){
      console.log(err);
    });
  ...
  buyTeamOneFun:function(gameid,one,two,three,buyCount,price)
  {
    return new Promise((resolve,reject)=>{
      ...
      web3.eth.getAccounts(function(error, accounts) {
        ...
        App.contracts.TutorialToken.deployed().then(function(instance) {
          tutorialTokenInstance = instance;
          // var end_price =  price / 1000000000000000000 ;
          //uint256 _gameID, uint256 _one, uint256 _two, uint256 _three, uint256 _buyCount
         let initPrice =  web3.toWei(price,'ether');
         //加上5%的手续费
         var totalPrice = (initPrice * 0.05) +  initPrice * 1;
         let count = parseInt(buyCount);
          return tutorialTokenInstance.buy( parseInt(gameid), parseInt(one), parseInt(two), parseInt(three),  parseInt(count) ,totalPrice, {from: account, value: totalPrice, gas:700000});
        }).then(function(result) {
          resolve(result);
        }).catch(function(err) {
          reject(err);
...
//------------------------玩法一购买记录----------------------
  getTraddingOne: function(event) {
    ...
    var end = document.getElementById("gettraddingOne_latest").value;
     ...
     App.getTraddingOneFun(address,begin,end).then(function(result){
      console.log(result);
    }).catch(function(err){
      console.log(err.message);
    });
  ...
  getTraddingOneFun:function (address,start,end) {
    return new Promise((resolve,reject)=>{
      ...
      App.contracts.TutorialToken.deployed().then(function(instance) {
        ...
        var myEventBouns = tutorialTokenInstance.BuyWinner(adr, {fromBlock: start, toBlock: end, gas:700000});//{curOwner: App.loginaccount}, 'latest'
...
//------------------------玩法二购买记录----------------------
    getTraddingTwo: function(event) {
      ...
      App.getTraddingTwoFun(address,begin,end).then(function(result){
        console.log(result);
      }).catch(function(err){
        console.log(err.message);
      });
    ...
    getTraddingTwoFun:function(address,start,end)
    {
      return new Promise((resolve,reject)=>{
        ...
        App.contracts.TutorialToken.deployed().then(function(instance) {
            ...
            // 获取事件对象
        let listData  = new Array();
        var myEventBouns = tutorialTokenInstance.BuyWinnerTwo(adr, {fromBlock: start, toBlock: end});//{curOwner: App.loginaccount}, 'latest'
        myEventBouns.watch(function(err, result) {
          if (!err) {
            let info =  result['args'];
            listData.push(info)
    ...
    //------------------------奖金池分配记录----------------------
      ShareBonus: function(event) {
        ...
        //调用拉取
        App.ShareBonusFun('',0,'latest').then(function(result){
         console.log(result);
       }).catch(function(err){
         console.log(err.message);
       });
      ...
      ShareBonusFun:function(address,start,end)
      {
        return new Promise((resolve,reject)=>{
          ...
          var myEventBouns = tutorialTokenInstance.ShareBonus(adr, {fromBlock: start, toBlock: end});//{curOwner: App.loginaccount}, 'latest'
          myEventBouns.watch(function(err, result) {
            if (!err) {
            let info =  result['args'];
              shareList.push(info);
...
//-------------------------查询玩法一中奖用户--------------------
    getWinnersOneFun:function(param)
    {
      return new Promise((resolve,reject)=>{
        ...
        var myEventBouns = tutorialTokenInstance.BuyWinnerList(param, {fromBlock:0, toBlock:'latest'});//{curOwner: App.loginaccount}, 'latest'
        myEventBouns.watch(function(err, result) {
          if (!err) {
            let info = result['args'];
      ...
      getWinnersTwo:function(event)
      {
        ...
        App.getWinnersTwoFun(gameid,teameid).then(function(result)
      {
        //解析数据
    ...
    getWinnersTwoFun:function(gameId,teameId)
    {
      return new Promise((resolve,reject)=>{
        ...
        App.contracts.TutorialToken.deployed().then(function(instance) {
    		...
    		var myEventBouns = tutorialTokenInstance.BuyWinnerTwo(adr, {fromBlock:0, toBlock:'latest'});//{curOwner: App.loginaccount}, 'latest'
    		myEventBouns.watch(function(err, result) {
    			if (!err) {
            // resolve(r;esult);
    ...
    SharesAmountFun:function(winers,amounts,gameId,type,total)
    {
      return new Promise ((resolve,reject)=>{
        ...
        web3.eth.getAccounts(function(error, accounts) {
          ...
          App.contracts.TutorialToken.deployed().then(function(instance) {
            tutorialTokenInstance = instance;
        return  tutorialTokenInstance.batchShareAmount(winers,amounts,gameId,type,total,{from: account, gas:700000});
        }).then(function(result) {
            resolve(result);
          }).catch(function(err) {
            reject(err);
...
//-------------------玩法一分享奖金-------------------
      calculatePriceOne:function(data)
      {
        ...
        //获得本场比赛所有奖金
        App.getListTeamByIdFun(gameid).then(function(result){
          //本场比赛总金额
          ...
          //分配奖金
           App.SharesAmountFun(userAddress,userPrice,gameid,type,lastToal).then(function(result){
             //分完奖金结束本场比赛
...
//--------------------玩法二分享奖金------------------
      calculatePriceTwo:function(data)
      {
        ...
        App.getListTeamByIdFun(gameid).then(function(result){
          ...
          //分配奖金
           App.SharesAmountFun(arryAddress,userPrice,gameid,type,last_taotal).then(function(result){
             //分完奖金结束本场比赛
             App.stopGameFun(gameid);
           });
        });
...
//----------结束某场比赛----------
    stopGame:function(event)
    {
      ...
      App.stopGameFun(gameId).then(function(result){
        console.log(result);
      }).catch(function(err){
        console.log(err.message);
      });
...
//----------------------结束比赛场次---------
      stopGameFun:function(gameid)
      {
        return new Promise((resolve,reject)=> {
          ...
          App.contracts.TutorialToken.deployed().then(function(instance) {
              tutorialTokenInstance = instance;
              return tutorialTokenInstance.setTeamStatus(false,gameid,{from: account, gas:700000});
        }).then(function(result) {
          console.log(result);
            resolve(result);
        }).catch(function(err) {
